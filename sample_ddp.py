...
def main(args):
    ...
    state_dict = find_model(ckpt_path, mode=args.checkpoint_mode)
    ...
    dist.barrier()
    if rank == 0:
        sample_file_name = create_npz_from_sample_folder(sample_folder_dir, args.num_fid_samples)
        print("Done.")
    dist.barrier()
    # dist.destroy_process_group()
    if rank == 0 and args.eval:
        from evaluator import metric
        metric(args.refer_file_name, sample_file_name)
    dist.destroy_process_group()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--refer_file_name", type=str, default="/mnt/workspace/diffusion/evaluations/VIRTUAL_imagenet300_256.npz")
